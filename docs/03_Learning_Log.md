...
하지만 프로토타입을 시각화하기 전에 해결해야 할 문제가 있다면 ELT 마지막 단계인 `dbt test`가 자그마치 거의 2시간 정도의 시간이 소모된다는 것입니다. 이는 프로덕션 단계에서 가장 큰 병목이 될 가능성이 높습니다.
...
제가 설정해 놓은 테스트가 41개의 테스트 케이스므로 단순 계산으로 VIEW에서 정의한 연산을 41번 실행한 것이 됩니다. 거기에 테스트는 계속 늘어나기만 할텐데 지금보다 엄청난 크기의 병목이 되겠죠.

### 해결

해결책으로 간단한 방법이 존재했는데요. 바로 `dbt`의 Materialization(구체화) 설정을 변경하는 것입니다. 로컬 테스트는 데이터 세트가 작으므로 모두 VIEW로 생성해 무결성을 확인하고, 프로덕션일 때는 테이블로 생성해 사전 연산으로 저장해 놓은 상태에서 테스트를 진행하는 것입니다. 예시는 다음과 같습니다.
