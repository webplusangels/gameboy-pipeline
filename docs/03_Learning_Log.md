...
`dbt`에서 `read_json_auto`는 S3 링크를 알아서 잘 읽는 것 같습니다. 와일드카드(`*`) 또한 자동으로 읽어 여러 개의 json을 한 번에 처리하기도 합니다. 매우 편리하지요.
...
`dbt run` 단계에서 6회의 테이블 조인을 실행합니다. 이 과정에서 반복적으로 S3에서 game 데이터를 가져와야 하고, 결국 500MB 정도 되는 데이터 파일을 여러 번 읽으면서 몇 GB가 사용됩니다. 더 큰 문제는 테스트에 있는데 테스트 스위트는 총 41개이고, 현재는 아니지만 초기 테스트에선 매 테스트마다 게임 테이블을 가져와야 했으므로, 얼추 계산해도 20GB 정도의 데이터 전송이 사용됩니다. 그럼 순식간에 4000원 정도가 날아갑니다.
...
## [Orchestration] 백필(Backfill)의 의미는?

2025-11-12

> [!WARNING]
> Backfill이 필요한 이유와, 필요없는 이유는?

### 문제

백필(Backfill)에 대해서는 이번 프로젝트를 진행하며 처음 들어보았습니다. 데이터 엔지니어링에서 매우 중요하고 흔하게 사용되는 용어 중 하나라고 하는데요. 쉽게 말해 '과거의 특정 시점으로 돌아가 파이프라인을 다시 실행시켜 데이터를 채워넣는 작업'을 의미합니다.

이 작업이 필요한 이유는 역시 시간 순서대로 쌓이는 데이터에서 중간에 비어있는 구간이나 잘못된 구간을 다시 채우는 과정이겠죠. 특히 큰 규모의 시스템에서는 다음과 같은 이유로 Backfill이 필수적인데요.

1. 장애 복구: 서버가 다운되거나 네트워크 오류로 인해 파이프라인이 실패했을 때
2. 로직 변경: 버그를 수정하거나 운영 상의 이유로 로직을 변경했을 때, 오류 시점부터 다시 계산해야 정확한 리포트를 확인할 수 있음
3. 새로운 지표 추가: 새로운 지표를 추가할 때 과거의 데이터를 분석하기 위해

### 해결

하지만 이 프로젝트에서는 지원하지 않습니다. 단순하게 생각했을 때, 이 기능이 필요하지 않기 때문인데요. 정리하자면 다음과 같은 이유로 굳이 구현하지 않기로 했습니다.

1. 비용 대비 효율: 전체 데이터 양이 적기 때문에 Full Refresh로 충분함
2. 구현 난이도: 파라미터를 변수로 받고(이미 구현되어 있지만), 멱등성을 파이프라인 시스템에서 보장해야 함(별도의 테스트 필요)
3. 비용: Full Refresh로 충분하지 않다고 가정해도, 인프라 측면에서 컴퓨팅 자원에 들어가는 비용을 감당할 여지가 없음

### 결과

때문에 이처럼 작은 프로젝트에서는 필요하다면 모든 데이터를 지우고 다시 넣는 것이 개발자 경험 측면에서 훨씬 뛰어납니다. 하지만 데이터가 커지면 Full Refresh가 불가능하므로, 그 때부터는 Backfill이 가능한 구조로 파이프라인을 재설계하는 결정이 필요해보입니다.

## [Orchestration] 파이프라인 배포하기 전 검토 사항

2025-11-13

> [!WARNING]
> 파이프라인을 배포하기 전의 검토 사항은?

### 문제

파이프라인을 본격적으로 배포하기 전에 돌아가는 모든 코드에 대해 점검을 거쳤습니다. 그 결과 다음과 같은 문제점이 발견되었습니다.

- 에러 핸들링 부재
- 타임아웃 설정
- CloudFront 캐시 무효화

조금은 과하거나 미묘한 지적도 존재했습니다.

- Manifest 파일 검증 누락
- stg가 ephemeral이면 매번 전체 manifest 읽어서 비효율적
- state 백업 복구

이 문제들에 대해 차례로 해결책을 알아보고 현실적인 방안을 적용해봅시다.

### 해결

**고친 지점**

에러 핸들링의 경우 파이프라인 실행에 있어서 엄밀히 말하면 단일 실패 지점은 있지만(실패하면 에러가 발생되지만), E-L과 T는 결국 CI 파이프라인에서 서로 연결되는 지점이 없기 때문에 E-L에서 에러가 발생해도 T까지 전파되지 않는다는 취약점이 있습니다. 이를 해결하기 위해서는 E-L 실행을 담당하는 `run_pipeline.py`에서 오류가 발생할 때 모든 시스템을 종료할 수 있는 대책을 마련하거나, `elt_pipeline.yml`에서 각 step이 실패 시에는 에러를 전파하고 실행을 종료할 수 있도록 바꿔주는 것입니다.

타임아웃 또한 GitHub Actions의 사용 시간 제한을 생각하면 설정하는 것이 안전합니다.

CloudFront 캐시 무효화의 경우, 제 S3에서는 활성화된 파일을 표시하기 위해 manifest 파일을 쓰기 때문에 CloudFront에서 제공하는 파일들이 과거의 파일들이면 안됩니다. 따라서 EL 단계에서 업데이트 후, `dbt`에서 원활하게 업데이트를 할 수 있도록 적재 후 꼭 캐시 무효화를 하는 단계가 필요합니다.

미묘한 지적에 대해서 알아보겠습니다.

**미묘한 지점**

Manifest 파일 검증 누락의 경우엔 생길 수 없는 경우를 재검사하는 것에 가깝습니다. 코드에 따르면, 업로드한 파일을 확인하고 S3 업로드가 성공할 경우에 완료 로그를 작성하기 때문에 파이썬 스크립트가 성공했을 때는 Manifest 파일이 무조건 있다고 봐야 합니다.

증분 전략의 불일치에 대해서는, 제가 다른 문서에서도 언급했지만 가장 무거운 `stg_games`를 제외하면 데이터가 적은 차원 테이블들만 존재하기 때문에 Ephemeral로 설정해도 좋다고 생각했습니다. stg 모델은 결국 Manifest에 있는 모든 파일을 다 읽어야 하기 때문에 모두 테이블로 만드는 것이 적은 차이지만 제 관점에서는 효율적이라고 보기 어려웠습니다.

State는 업데이트 시점을 기록하는 파일입니다. State를 백업하거나 복구하기 위해 방안을 마련하는 것은 사실 Backfill 전략에서도 언급했지만 State가 날아가더라도 Full Refresh가 20분 미만인 현재 백업 로직을 짤 만큼의 손해가 아닙니다. 이는 오버 엔지니어링이라고 판단됩니다.

### 결론

배포 전에 검토를 해보니 생각보다 고민이 필요한 부분이 아직 많이 남아있었습니다. 에러 핸들링의 경우 `elt_pipeline.yml`에 `set -e`와 타임아웃을 추가해 파이프라인의 유연성을 높였습니다. 또한 Loader 작업 이후 Cloudfront 캐시 무효화를 위해 IAM 권한을 업데이트하고 `run_pipeline.py`에 로직을 추가하였습니다. 다른 부분에 대해서는 추후 문제가 생긴다면 그 때 고민해보는 것으로 하겠습니다.

## [Deploy] incremental과 external_location 설정의 충돌

2025-11-14

> [!WARNING]
> Incremental과 `external_location` 설정의 충돌

### 문제

파이프라인을 가동하고 난 후의 산출물을 데이터 마트에 넣는 과정에서 문제가 생겼습니다.

예를 들어 어느 날 수집을 끝낸 후 최종 데이터를 생성했다고 가정하겠습니다. 이 데이터는 GitHub Actions에 저장될 수 없으므로 parquet 형태로 S3에 저장됩니다. 다음 날 새로운 데이터를 수집한 파이프라인은 어제 파일을 기준으로 증분 업데이트하려고 하지만 기준을 삼을 파일이 서버 내에 존재하지 않으므로 Incremental이 작동하지 않습니다. 다시 말해 DuckDB는 파이프라인을 가동하는 GitHub Actions 머신 내부에서 파일을 찾고 이를 기준으로 데이터를 추가해 업데이트하려고 하지만, 이는 영속성을 지원하지 않는 GitHub Actions의 한계 때문에 스크립트 실행 후 내부의 파일을 지워버리는 것입니다.

### 해결

이를 해결하기 위한 핵심에 대해 설명하겠습니다.

**증분 불가능성에 대해**

저는 데이터 마트 산출물로 parquet 포맷을 사용하고 있습니다. 일단 parquet의 장점은 압축률이 좋고, 호환성이 높다는 것입니다. 다시 말해 사실상 데이터 포맷의 표준입니다. 만약 우리가 `dbt`에서 생성한 데이터 마트 (`dim_games`)를 DuckDB 네이티브한 형태(`.db`)로 저장한다고 하면 실질적인 의미의 증분이 가능할 것입니다.

하지만 앞에서 언급한 parquet의 장점을 생각해보면, `.db` 형태는 비교적 압축률이 좋지 않고, 호환성이 좋지 않다고 해석할 수 있습니다. S3를 쓰고 있는 만큼 내부 인덱스나 메타데이터 문제로 용량이 커진 파일은 업로드/다운로드 시간에 직접적인 영향을 줄 수 있습니다. 그리고 해당 데이터 산출물을 다른 시스템 내에서 표현하거나 가공해야 하는 만큼 호환성에 문제가 있다면 불안정합니다. 이후에 Athena나 Spark와 같은 시스템에서도 쓸 수 있어야 한다면 `.db` 포맷을 고집하기는 어려워보입니다. 게다가 S3의 객체가 불변성이라는 특징을 가지고 있기 때문에, 특정 행을 고치거나 하는 것은 불가능한 방법입니다.

따라서 최선의 방법은 파이프라인 적재 단계에서는 이전에 S3에 저장한 Parquet 포맷을 가져와 데이터를 합치면서 새로운 Parquet 파일을 덮어쓰기 하는 것입니다. 이 방법은 현재 `Streamlit`과도 완벽하게 호환이 되고, 데이터가 생기다가 마는 경우가 없기 때문에 원자성에서도 문제가 없습니다.

정리하면 엄밀히 말해 증분을 한다고 보기는 어렵지만, 데이터 변환(T) 단계에서 기존의 데이터를 가져와서 증분된 데이터를 더하고 덮어쓰기하는 변형된 증분 방식을 사용합니다.

### 결과

결국 파이프라인 업데이트는 다음과 같은 과정을 거치게 됩니다.

```
State 확인 -> State의 시점을 기준으로 데이터 수집 -> 데이터 적재

-> 데이터 변환 -> 데이터 마트의 Parquet와 비교하고 합친 후 새로운 Parquet 생성 -> 데이터 마트에 덮어쓰기
```

## [Refactor] 동시성 구현을 위한 개념

2025-12-01

> [!WARNING]
> Python 동시성 구현을 위한 개념을 알아보자

### 문제

두번째 리팩토링의 대상은 주요 병목 지점 중 하나인 Extractor입니다. 기존 로직은 비동기 라이브러리를 사용하고 있음에도 Request -> Wait -> Response의 단일 스레드 동기식 흐름을 따릅니다. 총 레코드가 Games 기준 342,000개라고 가정하고 페이지 당 최대 요청 수가 500일 때 필요 요청수는 약 684회 정도이며 평균 RTT는 1.2초로 보면 실제 측정 결과와 크게 다르지 않은 $684 \times 1.2s \approx 820s$ 로 계산됩니다. [(344,880 기준 $812s$)](./refactoring/benchmarks/data/extract_sequential_20251203_174758.json)

IGDB는 API 요청 제한이 초당 4개이고 동시 요청 수는 8개인데요. 현재는 1주일마다 전체 데이터를 받아오는 'Full Refresh'때의 데이터 수집 시간을 단축시키는 것을 기대하고 있습니다.

하지만 수집기의 API Rate Limiter를 확장 가능하게 설계하면, 향후 데이터를 확장시킬 때에도 API마다 상이한 요청 제한을 효과적으로 준수하면서 효율적으로 데이터를 수집할 수 있을 것으로 예상되는 만큼 현재의 리팩토링이 개선하고자 하는 지점은 단순히 해당 지점만이 아니라는 것을 알 수 있습니다.

### 해결

현재 async/await을 이용한 비동기 처리가 구현되어 있지만 이를 적절히 사용하고 있다고 보기에는 어려운 부분이 있습니다. 예를 들어 Python의 GIL은 스크립트에서 스레드 하나 만을 실행시킬 수 있는 강력한 제한이지만 CPU를 쓰는 작업이 아니라면 이벤트 루프에 다른 작업을 거의 동시에 실행시키고 대기할 수 있다는 장점이 있습니다. 현재는 비동기 함수를 이용할 수 있는 환경에 불과할 뿐입니다.

단순하게 `asyncio.gather`로 수천 개의 요청을 동시에 보내기만 하면 메모리 부족이나 API 요청 제한이 걸리게 됩니다. 그리고 Python 3.11+에서는 `asyncio.gather`보다 안전한 `asyncio.TaskGroup`을 권장하기도 합니다. 해당 방법은 배치 단위 작업에서 생겨날 수 있는 예외를 효과적으로 처리할 수 있는 장점이 있고, 배치 내의 원자성을 보장할 수 있다는 안정성을 가지고 있습니다.

그렇기 때문에 현재는 배치를 사용하되, offset을 미리 계산하고 이를 4개의 요청으로 묶어 `TaskGroup`으로 한 번에 처리하는 방식을 사용하려고 합니다. 물론 Sliding Window와 같은 방식이 더 빠를 수 있겠지만 현재 로직 구현의 정도나 복잡성을 따져볼 때, 시간 대비 유의미한 차이가 날 것이라고 기대하기 힘들고 (데이터가 적기 때문에) 오류 가능성이 훨씬 크기 때문입니다.

때문에 Python에서 상호보완적인 라이브러리인 `aiolimiter.AsyncLimiter`와 `asyncio.Semaphore`로 구현합니다. `AsyncLimiter`는 초당 통과할 수 있는 토큰을 제한하는 것으로, 여기서는 Bucket에 토큰을 위한 자리가 있을 때에 한해 초당 4개의 토큰을 충전합니다. `Semaphore`는 동시에 요청할 수 있는 요청의 개수를 뜻하는 것으로 최대 8개의 동시 요청 수를 처리할 수 있는 IGDB에는 8개의 슬롯을 놓고 요청이 완료되는 즉시 해당 슬롯을 해제하고 `AsyncLimiter` Bucket의 토큰을 가져옵니다.

`AsyncLimiter`는 초당 토큰을 제한하는 것으로 카운팅을 하지 않고 초마다 Bucket을 채웁니다. 이 말은 특별한 해제가 필요 없다는 뜻입니다(자리가 있을 때마다 채우고 내보내기 때문에). 하지만 세마포어는 슬롯을 점유하고 있으므로 완료가 될 때는 명시적으로 해제해야만 합니다. 이를 `@asynccontextmanager`로 구현하고 `with`문을 통해 쓰기 편하게 작성할 수 있습니다.

### 결과

벤치마크 결과 속도는 약 [2.17배](./refactoring/benchmarks/data/extract_concurrent_20251203_172801.json) 정도 개선되었고 이는 데이터가 큰 game 엔티티에 한해서이며, 페이지가 데이터보다 큰 차원 데이터들의 경우에는 배치 크기 때문에 경미하지만 시간이 더 소요되는 경우도 있었습니다.
