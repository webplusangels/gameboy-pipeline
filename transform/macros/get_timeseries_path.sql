...
{%- set latest_partition_query = "SELECT DISTINCT regexp_extract(key, 'dt=([0-9]{4}-[0-9]{2}-[0-9]{2})/', 1) AS dt FROM s3_list_objects('s3://" ~ env_var('S3_BUCKET_NAME') ~ "/raw/" ~ entity_name ~ "/', 'dt=*/*.jsonl') ORDER BY dt DESC LIMIT 1" -%}
